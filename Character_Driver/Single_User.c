...
static int device_open(struct inode *i, struct file *f)
{
    spin_lock(&lock);
//    if (count ){
    if (owner == current->cred->uid) {
/*    if (count && 
            (owner != current->cred->uid)  && // allow user 
            (owner != current->cred->euid) && // allow whoever did su 
            !capable(CAP_DAC_OVERRIDE)) { // still allow root 
*/
        spin_unlock(&lock);
        return -EPERM;   /* can be -EBUSY */
    }
